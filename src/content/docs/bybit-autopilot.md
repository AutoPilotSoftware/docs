...
AutoPilot поддерживает множество автоматизированных действий для Bybit:
...
- **Продажа всех активов**: конвертация всех монет в USDT маркет-ордерами
...
:::danger[Сохраните секретную фразу!]
Показывается один раз. Потеряли фразу = потеряли доступ ко всем адресам и средствам.
:::
...
Уже есть адрес? AutoPilot использует его — дубликатов не будет.
:::
...
:::tip[Приватность]
Каждый аккаунт → свой адрес → общих адресов нет → связать аккаунты между собой невозможно.
:::
...
Автоматическое прохождение Puzzle Hunt задач на Bybit

| Параметр | Столбец | Описание |
|----------|---------|----------|
| **Обновляет** | `[RESULT] status` | `[PUZZLE] SUCCESS` |
...
flowchart TD
    ...
    subgraph TRADE ["📊 Трейдинг и Ивенты"]
        T["trading — Торговля"]
        TS["ts — TokenSplash"]
...
Файл `BybitAutoPilot.config` содержит основные параметры:
...
| `captcha_provider` | Провайдер капчи | `2captcha`, `capmonster` |
...
3. **Настройте** `BybitAutoPilot.config`:
   - Укажите `activation_key` (получен при покупке)
   - Укажите `captcha_key` (от [2captcha](https://2captcha.com/auth/register), [ruCaptcha](https://rucaptcha.com/auth/register) или [Capmonster](https://capmonster.cloud/Account/Signup))
