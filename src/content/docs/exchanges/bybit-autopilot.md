...
Быстрый вывод средств с аккаунта с автоматическим подтверждением (email + 2FA).
...
| Опционально | `[WITHDRAW] withdraw_amount` | Сумма в % от баланса Funding (1–100). Пусто или `100` = весь баланс, `50` = половина. Значения >100 отклоняются |
| **Обновляет** | `[RESULT] status` | `[WITHDRAW] SUCCESS` |

> Если 2FA не подключен — AutoPilot автоматически подключит его перед выводом.
>
> 💡 **Имя сети должно быть каноническим именем Bybit**, а не дружелюбным алиасом. Например, USDT-TRC20 в Bybit называется `TRX`, USDT-BEP20 — `BSC`, USDT-ERC20 — `ETH`, Polygon — `MATIC`. Если впишете не то имя — действие остановится с подсказкой и полным списком работающих сетей в логе.

#### Подсказка: какую сеть выбрать для USDT

| Сеть | Комиссия | Мин. вывод | Когда выбирать |
|------|----------|------------|----------------|
| `APTOS` | 0 USDT | от 0 | **Самый дешёвый и быстрый.** Используйте, если получатель поддерживает Aptos |
| `TRX` | 1 USDT | 2.6 | Самая узнаваемая сеть для USDT (TRC20). Поддерживается почти везде |
| `BSC` | 0.2 USDT | 10 | BNB Smart Chain (BEP20). Дёшево, широко поддерживается биржами |
| `MATIC` | 0.1 USDT | 1 | Polygon. Дёшево, удобно для DeFi |
| `ARBI` | 0.1 USDT | 1 | Arbitrum One. Низкая комиссия, L2 Ethereum |
| `OP` | 1 USDT | 1 | Optimism. L2 Ethereum |
| `SOL` | 0.5 USDT | 10 | Solana. Быстро |
| `ETH` | 0.8 USDT | 6 | Ethereum (ERC20). Дорого, используйте только если получатель не поддерживает другие сети |
| `TON` | 0.15 USDT | 1 | **Требует memo** — обязательно заполните `[WITHDRAW] withdraw_memo` |
