...
> ⚠️ В отличие от остальных торговых действий, параметры для `limit` читаются **из `BybitAutoPilot.config`**, а не из колонок `[TRADING]` Excel-таблицы.

**Параметры из `BybitAutoPilot.config`:**
...
- Имя файла — `BybitAutoPilot.config` (допустимо также `AutoPilot.config`), расположен рядом с исполняемым файлом.
...
```mermaid
flowchart TD
    A["Чтение параметров из BybitAutoPilot.config"] --> B["Запрос открытых ордеров аккаунта"]
