...
Быстрый HTTP-вывод средств без открытия браузера — прямые запросы к Bybit API, подтверждение email-кодом + 2FA, всё автоматически. Заменил старую браузерную реализацию по умолчанию (она оставлена как fallback только для `full_withdraw=YES + USDT`).
