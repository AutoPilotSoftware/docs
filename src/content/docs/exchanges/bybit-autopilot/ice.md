...
### 🛡️ `hedge_mode` — hedge или one-way (важно для риска)

Управляет режимом позиции на фьючерсах. **По умолчанию `YES` (hedge)** — так безопаснее.

| | `hedge_mode=YES` (hedge) | `hedge_mode=NO` (one-way) |
|---|---|---|
| Режим | двухсторонний (long и short раздельно) | односторонний (одна позиция на монету) |
| Твоя позиция по монете | **не трогает** — бот открывает свою отдельную ногу | ⚠️ фейд в обратную сторону **уменьшит/закроет** её |
| Переключение режима | нужно (тихо пропускается, если уже стоит) | не нужно вообще |
| Когда выбирать | **по умолчанию, всегда безопасно** | только если фьюч-акк **пустой** по этой монете |

> ⚠️ **`hedge_mode=NO` — только на чистом аккаунте.** В one-way, если у тебя уже открыта позиция по BTC, фейд-шорт бота **уменьшит или закроет** её. Режим `hedge` (по умолчанию) держит ногу бота отдельно и твои позиции не трогает.
...
ice_futures_hold=5,20
# Режим позиции: YES=hedge (двухсторонний, безопасно рядом с твоими позициями), NO=one-way (проще, но фейд схлопнет открытую позицию по монете — только на пустом акке).
hedge_mode=YES
