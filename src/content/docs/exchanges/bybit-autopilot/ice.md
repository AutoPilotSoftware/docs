...
- **`hedge_mode=YES` — dual (обе стороны сразу).** Открывает **одновременно long И short** одинакового размера, держит `ice_futures_hold`, закрывает обе. Стакан **не ждёт** (`ice_futures_wait` тут не нужен). **Дельта-нейтрал:** направление не важно (одна нога в плюс, другая в минус — гасят друг друга), объём **×2**, риск = только комиссии+спред. Нужно **2× маржи** на балансе. Это тот же приём, что в обычном action `futures`.
...
| Опционально `ice_futures_wait` | Сколько ждать появления крупного ордера (сек), потом пропуск монеты. **Только при `hedge_mode=NO`** (в dual стакан не ждётся) | `300` |
...
| Ждёт стакан? | да — ждёт топ-ордер ≥ notional (`ice_futures_wait`) | нет, входит сразу |
...
flowchart TB
    ...
    subgraph YES["hedge_mode = YES · dual (обе стороны)"]
        direction TB
        y1["Открывает СРАЗУ обе:<br/>LONG + SHORT"] --> y2["Дельта-нейтрал:<br/>объём ×2, риск = комиссии"]
    end
...
flowchart TD
    ...
    DU2 --> DU3["Открыть СРАЗУ LONG + SHORT маркетом"]
...
Здесь маржа 10 000–15 000 USDT × плечо 10 = позиция 100k–150k. При `hedge_mode=NO` бот ждёт в стакане BTC/ETH одиночный ордер такого размера и входит против него. При **`hedge_mode=YES`** вместо фейда откроет **сразу long+short** (нужно **2× маржи** на балансе, стакан не ждётся). **Для теста** ставьте `ice_futures_order=10,20` (позиция 100–200 USDT) и `ice_dryrun=YES`.
...
# Сколько ждать топ-ордер в стакане, сек. Не дождался → skip монеты. Только при hedge_mode=NO (в dual не ждётся).
ice_futures_wait=300
# Держать позицию перед закрытием, сек (диапазон, рандом). Работает в обоих режимах.
ice_futures_hold=5,20
# Режим входа: NO = one-way фейд (одна нога против стакана, по умолчанию). YES = dual — открывает СРАЗУ обе
# стороны (long+short), дельта-нейтрал, объём x2, риск = только комиссии+спред, нужно 2x маржи на балансе.
hedge_mode=NO
