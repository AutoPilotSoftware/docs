...
Управляет тем, **как биржа держит твою позицию** по монете. **По умолчанию `YES` (hedge).**
...
ice_futures_hold=5,20
# Режим позиции на фьючерсах: YES = hedge (long и short по монете раздельно), NO = one-way (одна нетто-позиция, проще, без переключения режима).
hedge_mode=YES
