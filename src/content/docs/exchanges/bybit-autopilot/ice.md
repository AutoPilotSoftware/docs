...
> 🔒 **Продажа гарантированно добивается.** Если из-за резкого движения против нас или тонкого стакана IOC-лимитка исполнилась не полностью, бот перечитывает баланс и **до-продаёт остаток** со всё более глубоким заходом (0.1% → 0.2% → 0.3%), пока монета не уйдёт в ноль (до 3 попыток). Так спот закрывается так же надёжно, как фьючерс маркетом. Рост цены продаже не мешает — наоборот, исполнится ещё выгоднее.
...
flowchart TD
    ...
    F --> G["SELL — лимит чуть ниже bid + IOC<br/>verify+retry, пока не продано"]
