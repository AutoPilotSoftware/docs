...
- **Обмен может не пройти — и это зависит от региона.** Биржа проверяет заявку, и если ваучер аккаунту не подходит, обмен отклоняется («Redeeming failed», причина — «несоответствие правилам промо»), а **поинты возвращаются**. Живьём (CIS-аккаунт): ваучер на **стоковые** перпы (Tesla/Nvidia и т.п.) отклонён, 400 поинтов вернулись — американские стоки недоступны в этом регионе. **Крипто**-ваучеры (BTC/ETH/SOL/XRP) и «безсимвольные» (скидка на комиссию, процент, Earn-бонус) чаще проходят — их и берём в первую очередь. **Но и это не гарантия:** на другом CIS-аккаунте (03.08.2026) отклонились **все** обмены подряд — и стоки, и сырьё, и крипто, и «безсимвольные», поинты вернулись. То есть у части регионо-залоченных аккаунтов обмен не проходит вообще: тогда ваучер этим путём не получить, и выбор ваучера под регион — часть логики, но не спасает, если весь обмен закрыт.
...
**Спотовые грид-боты на своих деньгах — сняты, разобраны и в продукте (03.08.2026).** Новое действие `spot_grid_bit` само собирает спот-грид на BTCUSDT из собственных USDT аккаунта (по умолчанию 20 USDT): берёт у биржи её же рекомендованный ценовой диапазон — чтобы бот сразу пошёл в работу, а не завис «в ожидании», как у ручных попыток раньше, — проверяет и создаёт. Это ровно задание «спот-грид в подарок на 120 USDT» (вложить ≥20 USDT, продержать сутки). Живьём: бот создан и **сразу «в работе»**.
