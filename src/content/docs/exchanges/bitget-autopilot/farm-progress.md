---
title: "🌾 Фарм наград Bitget — farm_bit, futures_bit, grid_bit"
description: "Три действия для Bitget: бесплатные награды и вход во все воронки, движок торгового объёма и ваучерные грид-боты."
sidebar:
  hidden: true
pagefind: false
---
...
Дальше цикл повторяется по дням: `farm_bit` берёт ежедневные задачи и чек-ин, `futures_bit` — дневной объём.
...
Полный круг открыть-закрыть отработан на реальном аккаунте 27.07.2026: обе сделки исполнились, маржа вернулась, сессия цела.
...
### Этап 3 — купоны и разовые задания · сделано, ждёт живого прогона

Приём купонных заданий и сбор разовых задач встроены в `farm_bit`. Разобраны по записи реального трафика; **на живом аккаунте этот конкретный проход ещё не гонялся** — только его составные части.

### Этап 4 — торговые боты · `grid_bit` · сделано, ждёт живого прогона

Создание ваучерного грид-бота разобрано по записи реального трафика целиком, включая ответ биржи с номером созданного бота. **Живьём нашим кодом ещё не запускалось.**
...
flowchart LR
    ...
    D --> E["⏳ Этап 5<br/>Обмен поинтов<br/>на USDT"]
    style A fill:#4CAF50,color:#fff,stroke:none,rx:8
    style B fill:#4CAF50,color:#fff,stroke:none,rx:8
    style C fill:#42A5F5,color:#fff,stroke:none,rx:8
    style D fill:#42A5F5,color:#fff,stroke:none,rx:8
    style E fill:#9E9E9E,color:#fff,stroke:none,rx:8
```
...
**Обмен поинтов на USDT.** Поинты копятся исправно — за шесть дней ручной работы набралось 1 223, и наш сбор берёт больше, чем брал человек. Но **курс обмена мы пока не видели ни разу**: экран обмена в разобранных записях не открывался. Пока это не снято, честная формулировка такая — поинты копятся, сумма в деньгах неизвестна.

Туда же: получение и применение ваучеров-«усилителей позиции» (самая крупная позиция по номиналу) и спотовые грид-боты на своих деньгах. Всё это упирается в одно и то же — нужно снять запись работы с этими экранами, после чего механика разбирается тем же способом, что и уже сделанное.

---

## 🧭 Куда это идёт дальше

Сейчас каждое действие — одиночный прогон: отработало, написало строку в Excel, забыло. Для одного аккаунта этого хватает, для пачки — уже нет: непонятно, какой аккаунт на каком дне цикла, где протухает ваучер, сколько объёма осталось до следующей ступени награды.

Следующий шаг — **снимок состояния фарма**: отдельное безопасное действие, которое ничего не меняет на бирже, а только читает и записывает, где аккаунт сейчас находится — день чек-ина, что уже забрано, что принято, что скоро сгорит. Копится в локальной базе рядом с программой, чтобы позже это можно было показать одним экраном по всей пачке профилей.
