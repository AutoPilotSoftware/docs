...
```
[FARM_STATE] SUCCESS - day 3, 5 claimable, 6/7 coupon(s) not accepted,
             2 usable voucher(s), 1/3 event(s) joined, 214.30 USDT
```

> ⚠️ **Чего в снимке намеренно нет — баланса поинтов.** Цифру «My points» с сайта мы пока не нашли ни в одной записи трафика: перебрали все ответы, подходящего поля нет. Показывать выдуманное число хуже, чем не показывать никакого, поэтому в отчёт идёт то, что действие реально начислило, а не «баланс».
...
> 🔍 **Проверка живьём (30.07.2026).** Взяли рабочий фарм-аккаунт и прочитали его состояние напрямую с биржи (только чтение): ваучеров — `0`, запущенных ботов — `0`, купонных карточек — `0`. Для «выработанного» аккаунта это ожидаемо и ровно объясняет, почему три экрана выше пока не сняты — их **нечем** открыть на пустом аккаунте. Чтобы снять и разобрать механику, нужен аккаунт **в середине воронки**: с непотраченными ваучерами, поинтами выше минимального порога обмена и живыми купонными заданиями. На одном таком прогоне снимаются все три сразу — дальше они разбираются тем же способом, что и уже готовое.
