...
| **Требует** | `tp_sl_percentages` (конфиг) | `TP,SL` в % — действие берёт **SL** (2-е число). 💡 Для хеджа рекомендуем `5,5` (SL 5%) |
...
| **Обновляет** | `[RESULT] status` | `[MEXC_FUTURES] SUCCESS` или причина ошибки |
...
> 🛡️ **Идеально для хеджа.** Открой противоположную сторону к своей основной позиции (`futures_side=short` против лонга, `long` против шорта) — и риск перекрыт. Почему заходит: вход по рынку мгновенный, комиссия = 0, авто-стоп сам ограничивает убыток, позиция держится без присмотра до стопа, а размер в % от баланса даёт **одинаковый хедж сразу на всех профилях**. **Рекомендуемый старт:** `leverage=10`, `futures_position_size=20`, `tp_sl_percentages=5,5`.
