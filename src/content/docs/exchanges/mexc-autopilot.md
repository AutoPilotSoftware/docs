...
| Параметр | Столбец | Описание |
|----------|---------|----------|
| **Требует** | `[TRADING] trading_coin` | Пара (например: `BTC_USDT` или `BTC`) |
| **Требует** | `[TRADING] trading_amount` | Объём (по умолчанию — маржа в USDT, см. ниже) |
| **Требует** | `[MEXC] leverage` | Кредитное плечо (например: `20`) |
| **Требует** | `[MEXC] side` | Направление: `long` или `short` |
| Опционально | `[MEXC] stop_pct` | Стоп-лосс в % движения цены (например: `90`) |
| Опционально | `[MEXC] volume_unit` | Единица объёма: `usdt_margin` (по умолч.) / `usdt_notional` / `contracts` |
| Опционально | `[MEXC] margin_mode` | Режим маржи: `cross` (по умолч.) / `isolated` |
...
> **Объём (`trading_amount`)** по умолчанию — это **маржа в USDT** (залог). Размер позиции = `маржа × плечо`. Переключается через `[MEXC] volume_unit`: `usdt_notional` — размер позиции в USDT, `contracts` — число контрактов напрямую.

> **Стоп-лосс (`stop_pct`)** считается от **движения цены**: для `long` стоп = `вход × (1 − stop_pct/100)`, для `short` = `вход × (1 + stop_pct/100)`. Без `stop_pct` позиция открывается без стопа.
