...
> ⚠️ **Почему обычную сессию Bitget уже не загрузить.** Bitget каждые несколько минут проверяет security-token и сверяет его с активной сессией, cookie и другими данными авторизации. Если на твоём устройстве и на другом эти данные расходятся — Bitget разлогинивает аккаунт, поднимает уровень риска и в итоге не даёт пройти проверку. Поэтому загрузка аккаунта сейчас работает только через AutoPilot Bitget: он постоянно подтягивает актуальные данные авторизации и обновляет нужные ключи и cookie, чтобы сессия оставалась живой и корректной. Обычные сессии для Bitget больше не подходят — и, судя по тому, как Bitget усиливает проверки, в обратную сторону это вряд ли изменится.
