...
> 💚 **Ни аккаунт, ни почта не тратятся.** Отказ приходит раньше, чем AutoPilot запрашивает код подтверждения, поэтому повторная попытка идёт на тот же адрес — ничего не сгорает.

**Провайдеры с поддержкой ротации:** NodeMaven, Proxyshard, DataImpulse, Bright Data.

> ⚠️ **Нужен «липкий» (sticky) тариф** — такой, где номер сессии стоит в логине. Если его там нет, менять нечего: AutoPilot предупредит и попросит заменить прокси. Хост сам по себе ничего не гарантирует — те же провайдеры продают и обычные логины без сессии.

> 🎯 **AutoPilot больше не путает плохой прокси с плохим аккаунтом.** Один отказавший IP — вопрос к прокси. Тот же аккаунт, отбитый через **два разных** выходных IP, — уже вопрос к аккаунту. Так вы не поменяете здоровый пул прокси и не удалите живой аккаунт из-за одной ошибки.
