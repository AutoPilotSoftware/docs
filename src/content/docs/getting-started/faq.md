...
1. `[PROFILE] profile_id` — ID профиля из AdsPower/Dolphin (AdsPower: колонка **ID**, а не «Номер» — [почему это важно](#8--adspower--dolphin--vision--afina))
...
Переключателя браузера в AutoPilot **нет** — антик определяется автоматически по формату `[PROFILE] profile_id` в таблице. Правила проверяются сверху вниз, срабатывает первое подходящее:
...
> ✅ **Следствие: антики можно смешивать в одной таблице.** Браузер определяется для **каждой строки отдельно**, поэтому один запуск может одновременно вести профили AdsPower, Dolphin и Afina. Перед стартом AutoPilot проверит, что запущены все задействованные приложения, и будет крутить профили параллельно (сколько сразу — задаёт `parallel_limit`).
>
> Исключение — **Vision**: `vision_config` глобальный и стоит первым в цепочке, поэтому он переводит на Vision **все** профили. Смешать Vision с остальными нельзя.

> ⚠️ **Самая частая ловушка.** AdsPower показывает два разных идентификатора: **«Номер»** (порядковый, только цифры) и **«ID»** (буквы и цифры, например `k1b0tidb`). Если скопировать «Номер», AutoPilot увидит одни цифры и пойдёт в **Dolphin** — вы получите ошибку `Dolphin local server offline - Launch Dolphin then retry`, хотя Dolphin у вас даже не установлен. Копируйте колонку **ID**.

> ⚠️ **Вторая ловушка — Afina без ключа.** Для Afina нужны **оба** условия сразу: UUID в `profile_id` **и** `afina_api_key` в конфиге. Если ключ не задан, строка не подойдёт ни под одно правило — UUID не проходит проверку «буквы и цифры» из-за дефисов — и профиль уедет в **AdsPower**. В итоге вы увидите ошибку AdsPower про профиль, которого в AdsPower нет. Проверьте, что `afina_api_key` заполнен в конфиге.

**AdsPower:**
- 📥 Установите свежую версию SunBrowser: Настройки → Локальные настройки
- 💳 Требуется **платная подписка** (минимум Base)
- 📤 Экспорт профилей: Выделите → Экспортировать → Номер, ID, Имя. В таблицу AutoPilot копируется **ID** (вида `k1b0tidb`), а **не** «Номер» — иначе профиль уедет в Dolphin (см. предупреждение выше)
...
| Ошибка | Решение |
|--------|---------|
| `Dolphin local server offline`, а Dolphin не установлен | В `profile_id` попал «Номер» AdsPower (только цифры) вместо **ID** — см. начало раздела |
