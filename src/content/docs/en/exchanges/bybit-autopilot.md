...
Automatic account "warm-up" through the **Bybit Learn** section: the action sets a random profile avatar and works through learning articles, performing the full engagement set on each one (like, view, comment, add to favorites, share). This makes the account look more "alive" and counts toward progress in the Bybit Learn campaign — tasks for commenting on and sharing articles earn points and rewards.
...
Keep the `bybit_learn_links.txt` file in the pilot's folder — **next to `AutoPilot.exe`**. The action reads it on every run and takes **5 random** links. The file is editable: add or remove `learn.bybit.com` article links; empty lines and lines without `http` are ignored.

> The domain in the links is swapped automatically to match the profile's `[BYBIT] domain`, so you can keep links in the file in `learn.bybit.com` format — they will redirect themselves to the correct regional domain.

📥 <a href="/docs/bybit_learn_links.txt" download>Download the ready-made <code>bybit_learn_links.txt</code></a> — the full list of 91 articles. Put it next to `AutoPilot.exe` and tweak it to your liking if needed.
...
| `captcha_key` | Paid provider API key — **optional**, backup only | `abc123...` |
| `captcha_provider` | Captcha provider (⭐ `capsolver`, `capmonster`, `2captcha`) — optional, detected from the key format | `capsolver` |
...
3. **Configure** `AutoPilot.config`:
   ...
   - `captcha_key` is **optional** — AutoPilot solves captcha itself, for free. A key is only a backup for rare types: ⭐ [CapSolver](https://www.capsolver.com/) (recommended), [CapMonster](https://capmonster.cloud/), [2Captcha](https://2captcha.com/). See [FAQ → section 4](/docs/en/faq/#4--proxy-and-captcha) for details
