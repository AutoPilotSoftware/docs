...
> **3 способи завантаження:** Afina Browser, AdsPower TXT, Вручну (проксі + куки). Виберіть зручний нижче.

---

## Швидкий старт: Afina Browser

> Використовуєте **Afina**? Це найбільш автоматизований спосіб — бот розпізнає формат сам.

**Крок 1.** Відкрийте Afina → виберіть профілі → **Export**.

**Крок 2.** Збережіть отриманий файл.

**Крок 3.** Відкрийте бот [@AutoPilotKYC_bot](https://t.me/AutoPilotKYC_bot), натисніть **Завантажити акаунти** → **Afina Browser** → надішліть файл як документ 📎.

> **Готово!** Бот розпізнає куки, проксі та User-Agent автоматично.
...
|-|-|-|
| **Afina Browser** | Ви використовуєте Afina | Легко |
| **AdsPower TXT** | Ви використовуєте AdsPower | Легко |
| **Вручну** | Проксі і куки окремо | Середньо |

```mermaid
flowchart TD
    A[Натискаєте Завантажити акаунти] --> B{Який браузер?}
    B -->|Afina| C0[Оберіть Afina Browser]
    B -->|AdsPower| C[Оберіть AdsPower TXT]
    B -->|Без анти-детекта| D[Оберіть Вручну]
    C0 --> E0[Надішліть файл Afina]
    C --> E[Надішліть один .txt файл]
    D --> F[Крок 1: Надішліть проксі текстом]
    F --> G[Крок 2: Надішліть .json файли куки]
    E0 --> H[Бот перевіряє акаунти]
    E --> H
    G --> H
...
#### MEXC після KYC просить Face Verification — це платно?

Якщо запит прийшов **у перші 30 хвилин після KYC** — реверифікація **безкоштовна**. Бот сам її відстежить і довипустить селлера на Face Scan. Якщо пізніше (години/дні) — це вже окремий платний REKYC. Детальніше — в [гайді пілота](/docs/ua/pilot-faq/).
