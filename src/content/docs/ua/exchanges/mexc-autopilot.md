...
| **Потребує** | `tp_sl_percentages` (конфiг) | `TP,SL` у % — дiя бере **SL** (2-ге число). 💡 Для хеджу рекомендуємо `5,5` (SL 5%) |
...
| **Оновлює** | `[RESULT] status` | `[MEXC_FUTURES] SUCCESS` або причина помилки |
...
> 🛡️ **Iдеально для хеджу.** Вiдкрий протилежну сторону до своєї основної позицiї (`futures_side=short` проти лонга, `long` проти шорта) — i ризик перекрито. Чому заходить: вхiд по ринку миттєвий, комiсiя = 0, авто-стоп сам обмежує збиток, позицiя тримається без нагляду до стопа, а розмiр у % вiд балансу дає **однаковий хедж одразу на всiх профiлях**. **Рекомендований старт:** `leverage=10`, `futures_position_size=20`, `tp_sl_percentages=5,5`.
