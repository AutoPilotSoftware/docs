...
description: "AutoPilot Software — автоматизацiя MEXC: реєстрацiя, торгiвля, виведення коштiв"
sidebar:
  order: 2
...
- **Розумнi ф'ючерси**: вiдкриття по ринку з автоматичним стоп-лоссом i утриманням позицiї
...
```mermaid
flowchart TD
    START["🎯 Обираєте ACTION у таблицi"] --> AUTO{"AutoPilot запущений?"}
    AUTO -->|Так| LOGIN{"Потрiбен вхiд?"}
    LOGIN -->|Так| DO_LOGIN["🔑 Автоматичний логiн"]
    LOGIN -->|Нi| TFA{"Потрiбен 2FA?"}
    DO_LOGIN --> TFA
    TFA -->|Не пiдключений| DO_2FA["🔐 Автопiдключення 2FA"]
    TFA -->|Пiдключений| ACTION["⚡ Виконання дiї"]
    DO_2FA --> ACTION
...
flowchart TD
    ...
    COIN --> BUY["📈 Купiвля по маркету"]
    BUY --> WAIT1["⏳ Випадкова затримка"]
    WAIT1 --> SELL["📉 Продаж по маркету"]
    SELL --> CHECK{"Усi цикли пройденi?"}
...
### `futures_mx` — Розумнi ф'ючерси (ринок + стоп-лосс)

Вiдкриття ф'ючерсної позицiї **по ринку** з автоматичним **стоп-лоссом** за один виклик. Позицiя тримається до спрацювання стопа — без тейк-профiту i без зворотного закриття.

```mermaid
flowchart TD
    START["⚡ Запуск"] --> SPEC["📋 Параметри контракту"]
    SPEC --> PRICE["💹 Поточна цiна"]
    PRICE --> CALC["🧮 Розрахунок об'єму i цiни стопа"]
    CALC --> LEV["⚙️ Встановлення плеча"]
    LEV --> OPEN["📈 Вiдкриття по РИНКУ + стоп-лосс"]
    OPEN --> HOLD["⏳ Позицiя тримається до стопа"]
...
|----------|--------------|------|
| **Потребує** | `[TRADING] trading_coin` (Excel) | Пара (наприклад: `SOL_USDT` або `SOL`) — єдиний параметр на профiль |
| **Потребує** | `leverage` (конфiг) | Кредитне плече (наприклад: `15`) |
| **Потребує** | `futures_position_size` (конфiг) | Розмiр у % вiд ф'ючерсного балансу USDT (як маржа). Наприклад: `25` |
| **Потребує** | `tp_sl_percentages` (конфiг) | `TP,SL` у % — дiя бере **SL** (2-ге число). 💡 Для хеджу рекомендуємо `5,5` (SL 5%) — дає позицiї ~95% запасу ходу, щоб хедж дожив до кiнця i його не вибило завчасно |
| Опцiонально | `futures_side` (конфiг) | Напрямок: `long` (за замовч.) / `short` |
| Опцiонально | `futures_margin_mode` (конфiг) | Режим маржi: `cross` (за замовч.) / `isolated` |
| **Оновлює** | `[MEXC_FUTURES] order_id` | ID вiдкритого ордера |
| **Оновлює** | `[RESULT] status` | Записує **яка угода вiдкрита** — наприклад `[MEXC_FUTURES] SUCCESS - LONG SOL_USDT 5cont 10x SL@142.30 ~75.00USDT id=...` (напрямок / монета / контракти / плече / стоп / об'єм), або причина помилки. Видно прямо в стовпцi статусу — не заплутаєшся. |

> **Єдиний конфiг — жодних нових стовпцiв.** Усi параметри, крiм монети, беруться зi спiльного файлу конфiгурацiї (тi самi ключi, що й у ф'ючерсних дiй Bybit). На профiль в Excel задається лише монета.

> **Вiдкриття по ринку, а не лiмiткою.** На MEXC комiсiя ф'ючерсiв = 0, маркет-ордер зараховує торговий об'єм i входить миттєво.

> 🛡️ **Iдеально для хеджу.** Хедж вiдкривається **проти позицiї на iншiй бiржi** (основна позицiя там — тут, на MEXC, ти вiдкриваєш протилежну сторону: `futures_side=short` проти лонга, `long` проти шорта), i цiновий ризик перекрито. Чому заходить: вхiд по ринку миттєвий, комiсiя = 0, авто-стоп сам обмежує збиток, позицiя тримається без нагляду до стопа, а розмiр у % вiд балансу дає **однаковий хедж одразу на всiх профiлях**. **Чому SL 5%:** дає хеджу ~95% запасу ходу — позицiя доживає до кiнця, її не вибиває завчасно. **Рекомендований старт:** `leverage=10`, `futures_position_size=20`, `tp_sl_percentages=5,5`.

> **Розмiр позицiї (`futures_position_size`)** — це **% вiд доступного ф'ючерсного балансу USDT**, використовується як **маржа**. Розмiр позицiї = `маржа × плече`. Приклад: баланс 100 USDT, `futures_position_size=25`, `leverage=15` → маржа 25 USDT, позицiя ~375 USDT.

> **Стоп-лосс** береться з `tp_sl_percentages` (2-ге число = SL) i рахується вiд **руху цiни**: для `long` стоп = `вхiд × (1 − SL/100)`, для `short` = `вхiд × (1 + SL/100)`. Без SL позицiя вiдкривається без стопа.

> **Утримання.** Зворотного закриття немає — позицiя тримається скiльки завгодно, доки не спрацює стоп.

> ⚠️ Акаунти пiд ризик-контролем MEXC (повiдомлення «Position opening is unavailable…», код `6026`) не можуть вiдкривати позицiї до проходження верифiкацiї — дiя повiдомить про це в статусi.

---

### `link_mx` — Отримати посилання KYC верифiкацiї

> ⚠️ **Окрема дiя не потрiбна.** Отримання KYC-посилання для MEXC переїхало в нашу екосистему — використовуйте бота та розширення нижче.
...
flowchart TD
    subgraph REG ["🟢 Початок роботи"]
        R["register_mx — Реєстрацiя"]
        L["login_mx — Логiн"]
        C["check_mx — Перевiрка"]
    end

    subgraph SEC ["🔐 Безпека"]
        FA["2fa_mx — 2FA захист"]
        API["api_mx — API ключi"]
    end
    ...
    subgraph TRADE ["📊 Трейдинг"]
        T["trading_mx — Торгiвля"]
        FU["futures_mx — Розумнi ф'ючерси"]
    end
