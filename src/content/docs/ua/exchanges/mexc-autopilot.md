...
| Параметр | Стовпець | Опис |
|----------|----------|------|
| **Потребує** | `[TRADING] trading_coin` | Пара (наприклад: `BTC_USDT` або `BTC`) |
| **Потребує** | `[TRADING] trading_amount` | Об'єм (за замовчуванням — маржа в USDT, див. нижче) |
| **Потребує** | `[MEXC] leverage` | Кредитне плече (наприклад: `20`) |
| **Потребує** | `[MEXC] side` | Напрямок: `long` або `short` |
| Опцiонально | `[MEXC] stop_pct` | Стоп-лосс у % руху цiни (наприклад: `90`) |
| Опцiонально | `[MEXC] volume_unit` | Одиниця об'єму: `usdt_margin` (за замовч.) / `usdt_notional` / `contracts` |
| Опцiонально | `[MEXC] margin_mode` | Режим маржi: `cross` (за замовч.) / `isolated` |
...
> **Об'єм (`trading_amount`)** за замовчуванням — це **маржа в USDT** (застава). Розмiр позицiї = `маржа × плече`. Перемикається через `[MEXC] volume_unit`: `usdt_notional` — розмiр позицiї в USDT, `contracts` — кiлькiсть контрактiв напряму.

> **Стоп-лосс (`stop_pct`)** рахується вiд **руху цiни**: для `long` стоп = `вхiд × (1 − stop_pct/100)`, для `short` = `вхiд × (1 + stop_pct/100)`. Без `stop_pct` позицiя вiдкривається без стопа.
