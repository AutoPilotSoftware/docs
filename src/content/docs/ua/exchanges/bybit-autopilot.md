...
Швидке виведення коштiв з акаунта з автоматичним пiдтвердженням (email + 2FA).
...
| Опцiонально | `[WITHDRAW] withdraw_amount` | Сума у % вiд балансу Funding (1–100). Порожньо або `100` = весь баланс, `50` = половина. Значення >100 вiдхиляються |
| **Оновлює** | `[RESULT] status` | `[WITHDRAW] SUCCESS` |

> Якщо 2FA не пiдключено — AutoPilot автоматично пiдключить його перед виведенням.
>
> 💡 **Iм'я мережi має бути канонiчним iм'ям Bybit**, а не дружнiм алiасом. Наприклад, USDT-TRC20 у Bybit називається `TRX`, USDT-BEP20 — `BSC`, USDT-ERC20 — `ETH`, Polygon — `MATIC`. Якщо вписати не те iм'я — дiя зупиниться з пiдказкою та повним списком робочих мереж у логi.

#### Пiдказка: яку мережу обрати для USDT

| Мережа | Комiсiя | Мiн. вивiд | Коли обирати |
|--------|---------|------------|--------------|
| `APTOS` | 0 USDT | вiд 0 | **Найдешевша та найшвидша.** Використовуйте, якщо одержувач пiдтримує Aptos |
| `TRX` | 1 USDT | 2.6 | Найвпiзнаванiша мережа для USDT (TRC20). Пiдтримується майже всюди |
| `BSC` | 0.2 USDT | 10 | BNB Smart Chain (BEP20). Дешево, широко пiдтримується бiржами |
| `MATIC` | 0.1 USDT | 1 | Polygon. Дешево, зручно для DeFi |
| `ARBI` | 0.1 USDT | 1 | Arbitrum One. Низька комiсiя, L2 Ethereum |
| `OP` | 1 USDT | 1 | Optimism. L2 Ethereum |
| `SOL` | 0.5 USDT | 10 | Solana. Швидко |
| `ETH` | 0.8 USDT | 6 | Ethereum (ERC20). Дорого, використовуйте тiльки якщо одержувач не пiдтримує iншi мережi |
| `TON` | 0.15 USDT | 1 | **Потребує memo** — обов'язково заповнiть `[WITHDRAW] withdraw_memo` |
