...
| **Потребує** | `[DEPOSIT] deposit_chain` | Мережа — **канонiчний код Bybit** (наприклад: `TRX` для TRC20, `BSC` для BEP20, `ETH` для ERC20). Повний список кодiв — нижче 👇 |
| **Оновлює** | `[DEPOSIT] deposit_address` | Адреса депозиту |
| **Оновлює** | `[RESULT] status` | `[DEPOSIT] SUCCESS` |

> ⚠️ **Пишiть код мережi — не «TRC20» / «Aptos» / «ERC20», i без пробiлiв.** Значення йде в Bybit як є. Невiрний код **або зайвий пробiл** (`BSC ` замiсть `BSC`) → Bybit поверне `ret_code 10016 svc error`, i адреса не прийде.

**Коди мереж USDT** (список Bybit може змiнюватися):

| Введiть у `deposit_chain` | Мережа на Bybit | Memo |
|---|---|---|
| `TRX` | TRON (TRC20) | — |
| `BSC` | BSC (BEP20) | — |
| `ETH` | Ethereum (ERC20) | — |
...
> 💡 Тi самi коди працюють у `whitelist_chain` та `withdraw_chain`. Якщо помилитесь — AutoPilot виведе актуальний список мереж прямо в лог.
...
| **Потребує** | `[WHITELIST] whitelist_chain` | Мережа — **канонiчний код Bybit** (`TRX`, `BSC`, `ETH`, `APTOS`…). Коди — у таблицi вище, у роздiлi `deposit` |
...
Автоматична участь у TokenSplash iвентах на Bybit. Дiя виконується у два кроки: **реєстрацiя в iвентi** та (опцiйно) **виконання завдання на торгiвлю**.

| Параметр | Стовпець | Опис |
|----------|----------|------|
| **Потребує** | `[TS] code` | Код iвента (з посилання `…/token-splash/detail?code=`) |
| **Оновлює** | `[RESULT] status` | Статус реєстрацiї/завдання (див. нижче) |

#### Як це працює

1. **Реєстрацiя (участь).** Пiлот за потреби сам логiниться, перевiряє KYC i натискає «Участвовать» через API бiржi. **Баланс для цього не потрiбен** — навiть акаунт з 0 USDT буде зареєстрований в iвентi.
2. **Завдання на торгiвлю** (керується конфiгом `ts_order`). Якщо на акаунтi є USDT, пiлот купує токен iвента та одразу продає його (1 купiвля + 1 продаж маркет-ордерами) — це закриває «завдання на депозит» iвента. Розмiр кожного ордера — випадкове число мiж двома значеннями `ts_order`.

#### Залежнiсть вiд балансу

- **Немає / 0 USDT** → пiлот лише реєструється в iвентi. Якщо зареєструватися не вдалося — пише причину в таблицю.
- **Є баланс** (вистачає на розмiр ордера; ~60 USDT iз запасом) → пiсля реєстрацiї ще й торгує купити/продати, щоб закрити завдання.
...
- `ts_order=50,55` → кожен ордер випадковий **50–55 USDT** (купити на ~50–55 i продати — завдання на торгiвлю виконано).
- `ts_order=0,0` → завдання на торгiвлю **пропустити**, лише зареєструватися.
- Коментар `(random 55-100)` у дефолтному конфiгу — просто приклад; реальний розмiр береться з ваших `MIN,MAX`.

#### Статуси в `[RESULT] status`

| Статус | Значення |
|--------|----------|
| `[TS] {токен} - SUCCESS REGISTERED` | Зареєстрований в iвентi |
| `[TS] {токен} - ALREADY JOINED` | Уже був зареєстрований ранiше |
| `[TS] FAIL - NO KYC` | Акаунт без верифiкацiї |
| `[TS] FAIL - NO CODE - fill column "[TS] code"` | Не заповнено `[TS] code` |
| `[TS] FAIL - Cannot get TokenSplash details` | Бiржа не повернула данi iвента (невiрний/прострочений код) |
| `[TS] FAIL {токен} - {причина}` | Не вдалося зареєструватися |

> Якщо реєстрацiя не пройшла, бiржа повертає причину вiдмови — **мiтку**. AutoPilot записує її в `[RESULT] status` у поле `{причина}`.
...
> ⚠️ **Дiя застарiла.** Вручну отримувати посилання через `link` бiльше не потрiбно — для верифiкацiй використовуйте бота та розширення нижче.
...
Тримайте файл `bybit_learn_links.txt` у папцi з пiлотом — **поруч iз `AutoPilot.exe`**. Дiя читає його при кожному запуску i бере **5 випадкових** посилань. Файл можна редагувати: додавайте або прибирайте посилання `learn.bybit.com`; порожнi рядки та рядки без `http` iгноруються.
...
📥 <a href="/docs/bybit_learn_links.txt" download>Завантажити готовий <code>bybit_learn_links.txt</code></a> — повний список зi 91 статтi. Покладiть файл поруч iз `AutoPilot.exe` та за бажанням вiдредагуйте пiд себе.
