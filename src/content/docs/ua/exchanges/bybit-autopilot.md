...
- **Отримання нагород**: купони, пакетне отримання, нагороди за активність
...
- **На кожній статті** послідовно відправляються: **лайк** → сигнал **перегляду** (VIEW) → **коментар** (випадкова фраза з 15) → **в обране** (зірка) → **репост** → сигнал **репосту** (SHARE). Коментар і репост додатково реєструються як завдання кампанії Learn для нарахування нагород.

#### Алгоритм роботи

```mermaid
flowchart TD
    A["Перевірка сесії та прав<br/>(BybitCheckPermissions)"] --> B{"Вже залогінений?"}
    ...
    D -- "Ні" --> E["Встановлення випадкового аватара<br/>GET avatar/groups → POST update/avatar"]
    E --> F
    F --> G["Захоплення campaign_id кампанії Learn"]
...
- `[LEARN] PARTIAL - Avatar: YES/NO, N/5 articles` — виконано не все: аватар не встановився, або одна зі статей завершилася помилкою (виняток під час завантаження/запиту зменшує лічильник до `N < 5`).
- `[LEARN] FAIL - ...` — не вдалося увійти (`CAN'T LOGIN`), немає посилань (`NO LINKS IN FILE`) або помилка читання файлу (`LINKS FILE ERROR`).

> Якщо на конкретній статті не вдається визначити `groupId`, лайк/коментар/обране/репост на ній пропускаються (надсилається лише сигнал VIEW), але стаття все одно вважається пройденою — на статус `SUCCESS` це не впливає.
...
Автоматичне отримання доступних купонів на акаунті
...
Отримання всіх доступних купонів у пакетному режимі
...
Отримання нагород з івентів активності, обхід верифікації обличчя
...
| `claim` | Отримання купонів | ✅ | — |
| `claim_batch` | Пакетне отримання купонів | ✅ | — |
| `claim_activity` | Нагороди за активність | ✅ | — |
| `profit` | Розрахунок прибутку | ✅ | — |
