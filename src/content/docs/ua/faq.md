...
:::caution[Важливо]
Після налаштування переадресації відправте тестовий лист, щоб переконатися, що листи доходять. Деякі сервіси активують переадресацію із затримкою до 24 годин.
:::

:::note[Антивірус]
Якщо виникають проблеми з підключенням до пошти — перевірте, чи не блокує антивірус IMAP-порти (993, 143).
:::

:::note[Немає вашого сервісу?]
Напишіть розробнику — його буде додано.
:::
...
:::note[Бекап]
`totp_secret_code` додатково зберігається в логи профілю (папка `/logs`) — навіть якщо випадково очистите таблицю, секретний ключ завжди можна знайти в логах.
:::

:::tip[Автопідключення]
Дії `whitelist` та `withdraw` автоматично підключать 2FA, якщо він ще не встановлений. Окремо запускати `2fa` не потрібно.
:::
...
:::tip[Рекомендація]
Для реєстрації та логіну достатньо FAST. Для трейдингу та дій на сторінках з антибот-захистом — MEDIUM.
:::
...
:::caution[Якщо не заповнити]
Капча буде вирішена з IP з пулу проксі вирішувача — це може викликати підозри на деяких біржах.
:::

:::note[Баланс капчі]
Поповніть баланс CapGuru/CapSolver перед запуском. Вартість ~1$ за 1000 рішень. Перевірити баланс можна на сайті сервісу.
:::

:::note[Помилки капчі]
Якщо капча постійно не вирішується — перевірте валідність ключа `captcha_key` в конфігу та баланс акаунта вирішувача.
:::
...
:::tip[Порада]
Стовпець `ACTION` заповнюйте тільки для тих профілів, які хочете автоматизувати. Порожній ACTION = профіль буде пропущено.
:::

:::note[Паролі]
Якщо `password` не заповнено — AutoPilot згенерує надійний пароль автоматично (8-30 символів, великі + малі + цифри + спецсимволи) і запише в таблицю.
:::

:::note[Excel закритий?]
Таблиця повинна бути закрита під час роботи AutoPilot. Якщо таблиця відкрита — софт попередить і зачекає закриття.
:::
...
:::note[Мінімальний ордер]
Переконайтеся, що на балансі достатньо USDT для мінімального ордера на біржі (зазвичай ~5 USDT).
:::
...
:::caution[Важливо]
`trading_amount` — це розмір позиції **з урахуванням плеча**, а не сума реально використовуваного балансу. Реальний баланс на одну угоду = `trading_amount / leverage`.
:::
...
:::caution[Ризики]
Ф'ючерсна торгівля з плечем пов'язана з ризиком ліквідації. Використовуйте помірне плече та невеликі суми для набиття обсягу.
:::

:::tip[Порада]
Якщо ордер не заповнюється протягом `limit_futures_order_timeout` — сигнал перераховується заново. Збільшіть `limit_futures_price_offset_ticks` для швидшого заповнення (але з менш вигідною ціною).
:::
...
:::note[Тип стовпця]
Зробіть стовпець `[PUZZLE] event_code` **текстовим** (не числовим), інакше Excel може обрізати довгі коди.
:::
...
:::tip[Порада]
Запускайте `puzzle_hunt` щодня — софт сам визначить, що вже зроблено, і виконає тільки відсутні кроки.
:::

:::tip[puzzle_social]
Якщо потрібно виконати тільки соцзавдання без торгівлі — використовуйте ACTION `puzzle_social`.
:::
...
:::tip[За замовчуванням]
Якщо `withdraw_amount` не заповнено — `earn` відправить **весь** доступний баланс USDT в пул, а `unearn` забере **все** з пулу.
:::

:::note[Фіксована сума]
Вкажіть число в `withdraw_amount` для стейкінгу/виведення конкретної суми. Якщо запитана сума перевищує доступний баланс — буде використано весь доступний баланс.
:::

:::note[Flexible Savings]
Це гнучкий стейкінг — кошти можна забрати в будь-який момент, відсотки нараховуються щодня. Виведення миттєве.
:::

:::tip[Автоматичний трансфер]
При `earn` кошти автоматично переводяться з Trading-рахунку на Funding перед стейкінгом — не потрібно робити це вручну.
:::
...
Якщо адреса виведення збігається з адресою в whitelist — верифікація кодами не потрібна (швидке виведення).
:::

:::note[Memo/Tag]
Якщо мережа вимагає memo (наприклад, TON, ATOM) — заповніть стовпець `withdraw_memo`. Якщо не вимагає — залиште порожнім.
:::
...
:::tip[Приклад]
У вас старі акаунти Bybit із залишками. Замість ручного створення адрес, введення їх по одній та збору з кожного гаманця — запустіть 3 дії і все на вашому мастер-гаманці. Нуль ручної роботи, нуль комісій, нуль зв'язків між акаунтами.
:::

:::note[Можна перезапускати]
Якщо акаунт вже має адресу, `whitelist_aptos_hd` використає ту саму — дублікатів не буде.
:::
...
:::tip[Оптимізація]
Встановіть Ublock/AdBlock у профілі для блокування реклами — сторінки завантажуватимуться швидше.
:::
...
:::tip[Антидетект]
Увімкніть `shuffle_order=YES` та `sleep_between_accounts=YES` з `parallel_limit=3-5` для природної поведінки.
:::
...
:::note[Лайфтайм]
Всі оновлення безкоштовні назавжди.
:::

:::danger[Не переривайте завантаження]
Якщо закрити софт під час завантаження — архів може бути пошкоджений. Дочекайтеся завершення.
:::
