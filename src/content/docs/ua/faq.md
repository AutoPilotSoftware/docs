...
> 🔧 **Помилки капчі:** якщо капча постійно не вирішується — перевірте валідність ключа `captcha_key` в конфігу та баланс акаунта на сайті провайдера. Якщо проблема тільки на одній біржі — спробуйте перемкнутись на іншого провайдера (наприклад, `capsolver` → `capmonster`).
