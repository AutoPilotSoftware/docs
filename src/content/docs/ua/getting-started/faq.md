...
> ⚠️ **Безпека:** селлер зберігає копію refresh-токена і теоретично може читати вашу пошту паралельно. Якщо це критично — змініть пароль Microsoft-акаунта (через перше поле комбо), це інвалідує всі refresh-токени, включно з вашим. Після цього потрібно заново оформити OAuth-консент через браузер. Для одноразових задач (KYC-верифікація) ротація зазвичай зайва.
