...
flowchart TD
    ...
    subgraph Setup ["⚙️ Ініціалізація"]
        Init["📋 Читання конфіга<br/>━━━━━━━<br/>leverage<br/>futures_position_size<br/>tp_sl_percentage"]
        SetLev["🔧 Установка плеча на біржі"]
        Init --> SetLev
    end

    subgraph SignalLoop ["🧠 Пошук сигналу"]
        Analyze["📊 Аналіз ринку<br/>orderbook + kline"]
        Algo{"🎲 direction_algorithm"}
        Simple["📈 simple<br/>за трендом"]
        Inverse["📉 inverse_simple<br/>проти тренду"]
        Advanced["🧪 advanced<br/>multi-factor"]
        Direction{"🧭 Напрямок?"}
    end

    subgraph Entry ["🎯 Вхід"]
        PlaceBuy["🟢 Лімітний BUY<br/>price - offset_ticks"]
        PlaceSell["🔴 Лімітний SELL<br/>price + offset_ticks"]
        Fill{"⏳ Заповнений?<br/>━━━━━━━<br/>order_timeout<br/>check_interval"}
        Cancel["❌ Скасувати ордер"]
    end

    subgraph Hold ["📈 Утримання"]
        SetTP["🎯 TP/SL<br/>±tp_sl_percentage%"]
        Monitor["👁️ Моніторинг<br/>hold_interval"]
        ExitCond{"💥 TP/SL hit<br/>або таймер?"}
    end

    subgraph ExitPhase ["🚪 Вихід"]
        MarketClose["🔄 Market close<br/>фіксація P&L"]
    end

    subgraph LoopCheck ["🔁 Цикл"]
        VolCheck{"🎯 Цільовий обсяг?"}
        Wait["💤 Пауза<br/>iteration_wait_interval"]
    end

    Done(["✅ Готово"])
    Error(["❌ Помилка<br/>нестача балансу /<br/>біржа відмовила"])
    ...
    Direction -->|Немає сигналу| Wait
    PlaceBuy --> Fill
    PlaceSell --> Fill
    Fill -->|Заповнений| SetTP
    Fill -->|Таймаут| Cancel
    Cancel --> Wait
    SetTP --> Monitor
    Monitor --> ExitCond
    ExitCond -->|Ні, чекаємо| Monitor
    ExitCond -->|Так| MarketClose
    MarketClose --> VolCheck
    VolCheck -->|Досягнуто| Done
    VolCheck -->|Не досягнуто| Wait
    Wait --> Analyze
...
- API-ключ береться з налаштувань Afina
