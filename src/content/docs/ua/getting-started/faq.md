...
1. `[PROFILE] profile_id` — ID профілю з AdsPower/Dolphin (AdsPower: колонка **ID**, а не «Номер» — [чому це важливо](#8--adspower--dolphin--vision--afina))
...
Перемикача браузера в AutoPilot **немає** — антик визначається автоматично за форматом `[PROFILE] profile_id` у таблиці. Правила перевіряються згори вниз, спрацьовує перше відповідне:
...
> ✅ **Наслідок: антики можна змішувати в одній таблиці.** Браузер визначається для **кожного рядка окремо**, тому один запуск може одночасно вести профілі AdsPower, Dolphin та Afina. Перед стартом AutoPilot перевірить, що запущені всі задіяні застосунки, і крутитиме профілі паралельно (скільки одразу — задає `parallel_limit`).
>
> Виняток — **Vision**: `vision_config` глобальний і стоїть першим у ланцюжку, тому він переводить на Vision **усі** профілі. Змішати Vision з рештою не можна.

> ⚠️ **Найчастіша пастка.** AdsPower показує два різні ідентифікатори: **«Номер»** (порядковий, тільки цифри) та **«ID»** (літери та цифри, напр. `k1b0tidb`). Якщо скопіювати «Номер», AutoPilot побачить самі цифри і піде в **Dolphin** — ви отримаєте помилку `Dolphin local server offline - Launch Dolphin then retry`, хоча Dolphin у вас навіть не встановлений. Копіюйте колонку **ID**.

> ⚠️ **Друга пастка — Afina без ключа.** Для Afina потрібні **обидві** умови одразу: UUID у `profile_id` **та** `afina_api_key` у конфігу. Якщо ключ не заданий, рядок не підійде під жодне правило — UUID не проходить перевірку «літери та цифри» через дефіси — і профіль поїде в **AdsPower**. У підсумку ви побачите помилку AdsPower щодо профілю, якого в AdsPower немає. Перевірте, що `afina_api_key` заповнений у конфігу.

**AdsPower:**
- 📥 Встановіть свіжу версію SunBrowser: Налаштування → Локальні налаштування
- 💳 Потрібна **платна підписка** (мінімум Base)
- 📤 Експорт профілів: Виділіть → Експортувати → Номер, ID, Ім'я. У таблицю AutoPilot копіюється **ID** (виду `k1b0tidb`), а **не** «Номер» — інакше профіль поїде в Dolphin (див. попередження вище)
...
| Помилка | Рішення |
|---------|---------|
| `Dolphin local server offline`, а Dolphin не встановлений | У `profile_id` потрапив «Номер» AdsPower (тільки цифри) замість **ID** — див. початок розділу |
