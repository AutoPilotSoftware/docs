...
> **Видео-гайд:** KYC для Bybit и MEXC без авторизации, без передачи ссылок, с повторной верификацией — [смотреть на YouTube](https://youtu.be/6zhf3ytgfkE)

> **3 способа загрузки:** Afina Browser, AdsPower TXT, Вручную (прокси + куки). Выберите подходящий вам ниже.

---

## Быстрый старт: Afina Browser

> Используете **Afina**? Это самый автоматизированный способ — бот распознаёт формат сам.

**Шаг 1.** Откройте Afina → выберите профили → **Export**.

**Шаг 2.** Сохраните полученный файл.

**Шаг 3.** Откройте бот [@AutoPilotKYC_bot](https://t.me/AutoPilotKYC_bot), нажмите **Загрузить аккаунты** → **Afina Browser** → отправьте файл как документ 📎.

> **Готово!** Бот распознает куки, прокси и User-Agent автоматически.
...
|-|-|-|
| **Afina Browser** | Вы используете Afina | Легко |
| **AdsPower TXT** | Вы используете AdsPower | Легко |
| **Вручную** | Прокси и куки по отдельности | Средне |

```mermaid
flowchart TD
    A[Нажимаете Загрузить аккаунты] --> B{Какой браузер?}
    B -->|Afina| C0[Выберите Afina Browser]
    B -->|AdsPower| C[Выберите AdsPower TXT]
    B -->|Нет анти-детекта| D[Выберите Вручную]
    C0 --> E0[Отправьте файл Afina]
    C --> E[Отправьте один .txt файл]
    D --> F[Шаг 1: Отправьте прокси текстом]
    F --> G[Шаг 2: Отправьте .json файлы куки]
    E0 --> H[Бот проверяет аккаунты]
    E --> H
    G --> H
...
## Безопасность: кто имеет доступ к моим аккаунтам?

**Воркеры (продавцы KYC) не имеют доступа к вашим аккаунтам.** Они получают только уникальную одноразовую ссылку для прохождения верификации. Всё, что они могут сделать — пройти KYC (подтвердить личность). Они не могут:

- Войти в ваш аккаунт
- Просматривать баланс или историю
- Совершать сделки или выводить средства
- Изменять настройки аккаунта

> Ваши куки и данные остаются только в системе бота и никогда не передаются воркерам.
...
#### MEXC после KYC просит Face Verification — это платно?

Если запрос пришёл **в первые 30 минут после KYC** — реверификация **бесплатная**. Бот сам её отследит и довыпустит селлера на Face Scan. Если позже (часы/дни) — это уже отдельный платный REKYC. Подробнее — в [гайде пилота](/docs/pilot-faq/).
