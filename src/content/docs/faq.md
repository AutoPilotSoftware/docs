...
> 💰 **Баланс капчи:** пополните баланс выбранного провайдера перед запуском. Стоимость ~$1 за 1000 решений. Проверить баланс можно на сайте сервиса.

> 🔧 **Ошибки капчи:** если капча постоянно не решается — проверьте валидность ключа `captcha_key` в конфиге и баланс аккаунта решателя. Если проблема только на одной бирже — попробуйте переключиться на другого провайдера (например, `capsolver` → `capmonster`).
